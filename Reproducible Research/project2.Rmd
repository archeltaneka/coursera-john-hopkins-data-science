...
When observing extreme weather events on health impact, we can divide it into `FATALITIES` and `INJURIES` as they are closely related to "health". The same goes to economic impact. We can take `PROPDMG` and `CROPDMG` to estimate it. From here, we could do a process called **feature engineering** to generate a new variable based on the existing and related variables.
```{r}
...
There is one more thing. The problem with this dataset is that according to NOAA, only 48 events are available on its website. In our case, there are still 219 number of events. Let's examine the exact problem.
```{r}
events <- unique(subset_data$EVTYPE)
print(dim(table(events)))
```
If we only look at the "Thunder" events, we do not only see "THUNDERSTORM" but also other events that have "Thunder" in them.
```{r}
events[grep('THUNDER', events)]
```
Unifying all of them is a bit of a hassle. **Our goal is to only analyze the most harmful events**. To simplify this, we only clean the events in the 95th percentile.
```{r}
subset_health <- with(subset_data, aggregate(health_impact ~ EVTYPE, FUN=sum))
subset(subset_health, health_impact > quantile(health_impact, prob=.95))
```
```{r}
subset_health <- with(subset_data, aggregate(eco_impact ~ EVTYPE, FUN=sum))
subset(subset_health, eco_impact > quantile(eco_impact, prob=.95))
```

In the 95th percentile, "HURRICANE/TYPHOON", "TSTM WIND", "HURRICANE", and "STORM SURGE" appears in the table. We need to unify them by replacing those words into something universal like "THUNDERSTORM WIND", "HURRICANE (TYPHOON)", and "STORM SURGE/TIDE"
```{r}
subset_data$EVTYPE[(subset_data$EVTYPE=='TSTM WIND')] <- 'THUNDERSTORM WIND'
subset_data$EVTYPE[(subset_data$EVTYPE=='HURRICANE/TYPHOON')] <- 'HURRICANE (TYPHOON)'
subset_data$EVTYPE[(subset_data$EVTYPE=='HURRICANE')] <- 'HURRICANE (TYPHOON)'
subset_data$EVTYPE[(subset_data$EVTYPE=='STORM SURGE')] <- 'STORM SURGE/TIDE'
```
...
```
For the biggest economy impact, "FLOOD" tops the chart followed by "HURRICANE (TYPHOON)" and "STORM SURGE/TIDE".
